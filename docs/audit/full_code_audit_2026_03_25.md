...
1. `AUD-20260325-003`
   `parse_sentence()` 是直接输入面，优先级最高。
2. `AUD-20260325-004`
   `add_action()` 问题同时带溢出和格式串风险，且修复范围小。
3. `AUD-20260325-001`
   `save_object()` 是核心对象持久化路径，风险高但改动要谨慎。
4. `AUD-20260325-005`
   属于恢复路径问题，建议和统计相关测试一起补。
5. `AUD-20260325-002`
   可以顺手并入 `save_object()` 同一批修复。

## 下一轮审计建议

- 继续扫 `src/packages/core/` 里未看过的文件系统和编辑器路径。
- 继续扫 `src/vm/internal/` 里所有固定缓冲区和 `error(buf)` 模式。
- 对 `src/packages/contrib/` 和 `src/packages/dwlib/` 做一次格式化字符串和边界检查专项。
- 条件允许时，用 Debug 构建配合定向输入做一次长字符串压力回归。

## 本轮验证

- `build_codex_review_fix` 重新编译 `lpc_tests` 通过。
- `ctest --output-on-failure -j 1` 结果为 `40/40` 全通过。
